"""
Verificador de mensajes WhatsApp v6 contra datos reales.
...
Verifica:
  ...
  2. Ausencia de terminos prohibidos (score, cluster, ML, pipeline, ranking,
     modelo, algoritmo, servicio, diagnostico, analisis, IA, plataforma)
  ...
  6. Output: verificacion_v6_report.json
"""
import pandas as pd
import re
import json
...
MSG_FILE = BASE_DIR / "output" / "mensajes_wsp_v6.txt"
REPORT_OUT = Path(__file__).parent / "verificacion_v6_report.json"
...
def parse_v6_blocks(content: str) -> list:
    """Parsea el archivo v6 txt en bloques de leads."""
...
def check_forbidden_terms(msg: str) -> list:
    ...
    t = msg.lower()
    found = []
    for term in _FORBIDDEN_TERMS + _FORBIDDEN_TERMS_V6:
        if term in t:
            found.append(term)
...
def verify():
    print("=== Verificador de Mensajes v6 ===\n")
    ...
    leads_parsed = parse_v6_blocks(content)
    ...
    for lead in leads_parsed:
        ...
        # --- 5. Verificar datos contra parquet ---
        ...
        # --- 5a. Verificar header metadata vs parquet ---
        ...
        # --- 5b. Verificar contenido del mensaje segun tipo de insight ---
        ...
        if insight == "rival_fuerte":
            # v6: "{rival} se adjudico {cnt} de las {total_lost} licitaciones
            #      que no les quedaron — cerca de ${money} que fueron a sus manos."
            m = re.search(
                r"se adjudic[oó] (\d+) de las (\d+) licitaciones", msg
            )
            if m:
            ...
            # v6: "{rival} les gano {cnt} veces en competencia directa
            ...
            # v6: "Con {n_lp} postulaciones LP y tasa de {wr}%, estan {gap}
            ...
            if m_opp:
                ...
                if opp_real > 0 and abs(opp_msg - opp_real) / opp_real > 0.20:
            ...
            # v6: "De {n_lp} licitaciones de alto valor, la tasa es {wr}%"
            ...
            if m_lost:
                ...
                if lost_msg != total_lost:
                    add_error(lnum, rut, empresa, "total_lost",
                              ...
                              f"Msg dice {lost_msg} perdidas vs real {total_lost}")
                    has_error = True
            # Optional: check gap if WR < industry
            m_gap = re.search(r"(\d+) puntos bajo", msg)
            if m_gap:
                gap_msg = int(m_gap.group(1))
                gap_real = int(round((INDUSTRY_WR_MEDIAN - wr_real) * 100))
            ...
            # v6: "Llevan {m} meses sin postular en Mercado Publico."
            ...
            if m_tenders and m_meses:
                ...
                if abs(tenders_msg - tenders_real_round) > 200:
                    add_error(lnum, rut, empresa, "tenders_estimate",
                              tenders_msg, tenders_real_round,
                              f"Msg dice {tenders_msg} licitaciones vs estimado {tenders_real_round}")
                    has_error = True

        elif insight == "wr_bajo_lp":
            # v6: "De {n_lp} postulaciones LP, la tasa es {wr}% — {gap} puntos bajo"
            ...
            # v6: "{total_lost} de {total_bids} licitaciones no se adjudicaron
            #      — tasa de perdida del {lr}%."
            m = re.search(
                r"(\d+) de (\d+) licitaciones no se adjudicaron", msg
            )
            if m:
                lost_msg = int(m.group(1))
            ...
            # v6: "{total_wins} adjudicadas de {total_bids} postulaciones
            ...
            # v6: "En {total_lost} de sus licitaciones no adjudicadas,
            ...
            # v6: "Revise su historial: {wins} adjudicadas de {bids}
    ...
    # --- Console output ---
    print(f"\n{'='*50}")
    print("RESULTADO VERIFICACION v6")
    print(f"{'='*50}")
